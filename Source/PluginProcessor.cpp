...
void SimpleEQAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    *rightChain.get<ChainPositions::Peak>().coefficients = *peakCoefficients;
    ...
    auto cutCoefficients = juce::dsp::FilterDesign<float>::designIIRHighpassHighOrderButterworthMethod(chainSettings.highCutFreq, getSampleRate(), 2 * (chainSettings.highCutSlope + 1));
...
ChainSettings getChainSettings(juce::AudioProcessorValueTreeState& apvts) {
    ...
    /*
    facendo questo:
        settings.lowCutFreq = apvts.getParameter("Low Cut Freq") -> getValue();
    otterrei valori normalizzati, mentre per i coefficienti dei filtri ho bisogno di numeri reali
    */
...
{
    // creates parameters on gui

    juce::AudioProcessorValueTreeState::ParameterLayout layout;
    
    layout.add(std::make_unique<juce::AudioParameterFloat>("Low Cut Freq", "Low Cut Freq", juce::NormalisableRange<float>(20.f, 20000.f, 1.f, 0.25f), 20.f));
    
    layout.add(std::make_unique<juce::AudioParameterFloat>("High Cut Freq", "High Cut Freq", juce::NormalisableRange<float>(20.f, 20000.f, 1.f, 0.25f), 20000.f));
